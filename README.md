...
## StatsD/DogStatsD application metrics
...
Environment-specific Kubernetes resources, API-key Secrets, StatsD enablement, and network policy are maintained in the deployment repository rather than this agent source repository.
...
The receiver is disabled by default. Its queues, packet size, tag sizes, aggregation memory, per-metric/global active-series counts, active-series TTL, encoded batch size, and independent spool are configurable with the `STATSD_*` environment variables or matching `--statsd-*` command-line flags. Legacy `DOGSTATSD_*` variables and `--dogstatsd-*` flags remain accepted for backward compatibility; canonical StatsD settings take precedence when both forms are provided.

Readiness tolerates short traffic bursts. Packet-queue or custom-spool utilization must remain at or above `STATSD_SATURATION_THRESHOLD` (default `0.8`) for `STATSD_SATURATION_DURATION` (default `5m`) before `/readyz` reports sustained saturation. The `packet_queue_utilization_ratio`, `packet_queue_saturated`, `spool_utilization_ratio`, and `spool_saturated` metrics expose this state.
...
During termination, UDP intake stops first and parser workers receive up to `STATSD_SHUTDOWN_DRAIN_TIMEOUT` (default `10s`) to drain accepted packets into aggregation. The pipeline then performs a final durable spool flush. `shutdown_pending_packets` records the queue present at shutdown, and `shutdown_dropped_packets_total` accounts for packets left when the drain deadline expires.
