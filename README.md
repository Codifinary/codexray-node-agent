...
The receiver is disabled by default. Its queues, packet size, tag sizes, aggregation memory, per-metric/global active-series counts, active-series TTL, encoded batch size, and independent spool are configurable with the `STATSD_*` environment variables or matching command-line flags.
